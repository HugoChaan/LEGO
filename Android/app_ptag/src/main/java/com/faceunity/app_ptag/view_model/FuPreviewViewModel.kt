...
import com.faceunity.core.entity.FUCoordinateVectorData
...
class FuPreviewViewModel : ViewModel() {
    ...
    fun rotateAvatar(offset: Float) {
        ...
        avatar.transForm.setRotateDelta(FUCoordinateVectorData(0f, 1f, 0f), offset)
    }
    ...
    fun rollAvatar(offset: Float) {
        ...
        thread {
            ...
            while (process <= 1.0 && !isCancelRollAvatar) {
                ...
                avatar.transForm.setRotateDelta(FUCoordinateVectorData(0f, 1f, 0f), moveX * (1.0f - interpolation))
                process += 0.005f //
                Thread.sleep(33)
            }
            isRollNow = false
