...
public abstract class QRCodeView extends RelativeLayout implements Camera.PreviewCallback ,OnScanBoxRectListener{
    ...
    public void onPreviewFrame(final byte[] data, final Camera camera) {
        ...
        if(mDelegate != null){
            mDelegate.onCameraPreViewFrame(data,camera);
        }
    ...
    void onPostParseData(ScanResult scanResult) {
        ...
        if (TextUtils.isEmpty(result)) {
            ...
            Log.d("test", "解析数据："+scanResult.getResult());
            mSpotAble = false;
            try {
                if (mDelegate != null) {
