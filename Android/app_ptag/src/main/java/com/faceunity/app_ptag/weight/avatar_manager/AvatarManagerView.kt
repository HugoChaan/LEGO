...
import com.faceunity.app_ptag.databinding.ViewActivityManagerBinding
...
@JvmOverloads  constructor(
    ...
    private val binding: ViewActivityManagerBinding

    var onItemClick: (item: FuAvatarWrapper) -> Unit = {}
    var onItemDelete: (item: FuAvatarWrapper) -> Unit = {}
    ...
    init {
        binding = ViewActivityManagerBinding.inflate(LayoutInflater.from(context), this, true)
        initView()
    }
    ...
    private val stickerTouchHelper = ItemTouchHelper(object : ItemTouchHelper.Callback() {
        ...
        private var dragListener: DragListener = object : DragListener {
            override fun deleteState(delete: Boolean) {
                (binding.avatarDeleteLayout).let {
                    if (delete) {
            ...
            override fun dragState(start: Boolean) {
                if (start) {
                    binding.avatarDeleteLayout.visible()
//                    VibratorUtil.vibrate(FUARVideoApp.sContext)
                } else {
                    binding.avatarDeleteLayout.invisible()
//                    saveCustomList()
                }
            }

            override fun remove(position: Int) {
                if (position == 0 && customStickerList.size == 1) {
                    ToastUtils.showFailureToast(context, "至少要有一个形象")
                    return
                }
...
//            FULog.d("当前操作：${viewHolder.adapterPosition}, 是否抬起手指：$isTouchUp")
            ...
            binding.avatarDeleteLayout.getLocationOnScreen(removeBtnXY)
...
//                    viewHolder.itemView.visibility = View.INVISIBLE
    ...
    init {
        stickerTouchHelper.attachToRecyclerView(binding.avatarRecyclerView)
    }
